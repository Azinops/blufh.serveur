...
void init_w(char adr_ip[])
{
    ...
    information_sur_la_destination.sin_port=htons(PORT_EMETTEUR); // Port écouté du serveur (33333)
}
void fermer_socket()
{
...
void init_soc_receptrice()
{
    ...
    if(PROTOCOLE==1)
    {
      ...
      information_sur_la_source.sin_port=htons(PORT_RECEPTEUR); // Ecoute sur le port 33333
...
char* recevoir_msg()
{
    ...
    while(erreur!=0) // Boucle tant qu'une demande de session (SYN) tcp n'a pas été reçu
        erreur=listen(id_de_la_socket2,TAILLE_SOCKET);
    if(AFFICHER_MSG)
    {
        printf("\nlisten      : OK");
    }
    ...
    nombre_de_caractere=recv(id_de_la_nouvelle_socket,buffer,TAILLE_SOCKET,0);
    if (nombre_de_caractere==SOCKET_ERROR)
    {
        if(AFFICHER_MSG)
        {
